...
### 2.8 数値例 (worked example)

PRS が拾わない SNP (例: 健常者と患者で頻度が同じ SNP_D) を、RFscore-G がどう扱うかの直感的な例:

![worked example](RFind-G_worked_example.png)

(SNP_D は β=0 のため PRS には影響しないが、個人偏差 fc では他 SNP と並んで rank 化される → 「当該疾患 SNP 以外を多く持つ個人」では top に入り、g1 (PGS) との overlap を希釈 → 結果的に **当該疾患 RFscore が相対的に下がる**)
